...
use crate::models::academic::survey::reference::question_varieties::data_objects::DataObject as ReferenceDataObject;
use crate::models::academic::survey::reference::question_varieties::validation::question_varieties::{
...
pub fn routes(ctx: &AppContext) -> Routes {
    Routes::new()
        .prefix("api/academic/survey/reference/question_varieties")
        .add("/list", get(list))
        .add(
            "/store",
            post(store).layer(AuthorizedLayer::new(
                ctx.clone(),
                "academic.survey.reference.question_varieties.index",
            )),
        )
        // .add(
        //     "/{id}",
