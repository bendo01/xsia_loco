...
use crate::models::academic::candidate::reference::registration_types::_entities::registration_types as ReferenceModel;
use crate::models::academic::candidate::reference::registration_types::data_objects::DataObject as ReferenceDataObject;
use crate::models::academic::candidate::reference::registration_types::validation::registration_types::{
...
pub async fn list(
    ...
    let datas = ReferenceModel::Entity::find()
        ...
        .filter(ReferenceModel::Column::InstitutionId.eq(institution_id))
...
pub async fn update(
    ...
    item.student_registration_id = Set(payload.student_registration_id);
    item.unit_id = Set(payload.unit_id);
    item.institution_id = Set(payload.institution_id);
    item.registration_category_id = Set(payload.registration_category_id);
...
pub fn routes(ctx: &AppContext) -> Routes {
    Routes::new()
        .prefix("api/academic/candidate/reference/registration_types")
        .add("/list/{institution_id}", get(list))
        .add(
            "/store",
            post(store).layer(AuthorizedLayer::new(
                ctx.clone(),
                "academic.candidate.reference.registration_types.index",
            )),
        )
        .add(
            "/{id}",
