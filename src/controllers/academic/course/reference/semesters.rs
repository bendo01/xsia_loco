...
use crate::models::academic::course::reference::semesters::data_objects::DataObject as ReferenceDataObject;
use crate::models::academic::course::reference::semesters::validation::semesters::{
...
pub async fn store(
    ...
    // Extract dates safely using ok_or instead of expect
    let start_date = payload
        .start_effective_date
        .ok_or_else(|| Error::BadRequest("Start effective date is required".into()))?;

    let end_date = payload
        .end_effective_date
        .ok_or_else(|| Error::BadRequest("End effective date is required".into()))?;
    ...
    let model = ReferenceModel::ActiveModel {
        ...
        is_odd: Set(payload.is_odd),
        start_effective_date: Set(Some(start_date)),
        end_effective_date: Set(Some(end_date)),
...
pub async fn update(
    ...
    item.is_odd = Set(payload.is_odd);
    item.start_effective_date = Set(Some(start_date));
    item.end_effective_date = Set(Some(end_date));
...
pub fn routes(ctx: &AppContext) -> Routes {
    Routes::new()
        .prefix("api/academic/course/reference/semesters")
        .add("/list", get(list))
        .add(
            "/store",
            post(store).layer(AuthorizedLayer::new(
                ctx.clone(),
                "academic.course.reference.semesters.index",
            )),
        )
        .add(
            "/{id}",
