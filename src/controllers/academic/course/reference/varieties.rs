...
use crate::models::academic::course::reference::varieties::data_objects::DataObject as ReferenceDataObject;
use crate::models::academic::course::reference::varieties::validation::varieties::{
...
pub async fn update(
    ...
    item.curriculum_type_id = Set(payload.curriculum_type_id);
    item.start_effective_date = Set(payload.start_effective_date);
    item.end_effective_date = Set(payload.end_effective_date);
...
pub fn routes(ctx: &AppContext) -> Routes {
    Routes::new()
        .prefix("api/academic/course/reference/varieties")
        .add("/list", get(list))
        .add(
            "/store",
            post(store).layer(AuthorizedLayer::new(
                ctx.clone(),
                "academic.course.reference.varieties.index",
            )),
        )
        .add(
            "/{id}",
