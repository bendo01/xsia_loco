...
use crate::models::person::reference::religions::data_objects::DataObject as ReferenceDataObject;
use crate::models::person::reference::religions::validation::religions::{
...
pub fn routes(ctx: &AppContext) -> Routes {
    Routes::new()
        .prefix("api/person/reference/religions")
        .add("/list", get(list))
        .add(
            "/store",
            post(store).layer(AuthorizedLayer::new(
                ctx.clone(),
                "person.reference.religions.index",
            )),
        )
        // .add(
        //     "/{id}",
