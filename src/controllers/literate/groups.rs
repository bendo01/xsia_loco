...
use crate::models::literate::groups::_entities::groups as ReferenceModel;
use crate::models::literate::groups::data_objects::DataObject as ReferenceDataObject;
use crate::models::literate::groups::validation::groups::{ModelValidation, ModelValidator};
...
pub fn routes(ctx: &AppContext) -> Routes {
    Routes::new()
        .prefix("api/literate/groups")
        .add("/list", get(list))
        .add(
            "/store",
            post(store).layer(AuthorizedLayer::new(ctx.clone(), "literate.groups.index")),
        )
        // .add(
        //     "/{id}",
        //     get(show).layer(AuthorizedLayer::new(ctx.clone(), "literate.groups.index")),
        // )
        .add(
            "/{id}",
            delete(destroy).layer(AuthorizedLayer::new(ctx.clone(), "literate.groups.index")),
        )
        .add(
            "/{id}",
            put(update).layer(AuthorizedLayer::new(ctx.clone(), "literate.groups.index")),
        )
        .add(
            "/",
            post(index).layer(AuthorizedLayer::new(ctx.clone(), "literate.groups.index")),
        )
}
