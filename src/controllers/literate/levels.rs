...
use crate::models::literate::levels::_entities::levels as ReferenceModel;
use crate::models::literate::levels::data_objects::DataObject as ReferenceDataObject;
use crate::models::literate::levels::validation::levels::{ModelValidation, ModelValidator};
...
pub fn routes(ctx: &AppContext) -> Routes {
    Routes::new()
        .prefix("api/literate/levels")
        .add("/list", get(list))
        .add(
            "/store",
            post(store).layer(AuthorizedLayer::new(ctx.clone(), "literate.levels.index")),
        )
        // .add(
        //     "/{id}",
        //     get(show).layer(AuthorizedLayer::new(ctx.clone(), "literate.levels.index")),
        // )
        .add(
            "/{id}",
            delete(destroy).layer(AuthorizedLayer::new(ctx.clone(), "literate.levels.index")),
        )
        .add(
            "/{id}",
            put(update).layer(AuthorizedLayer::new(ctx.clone(), "literate.levels.index")),
        )
        .add(
            "/",
            post(index).layer(AuthorizedLayer::new(ctx.clone(), "literate.levels.index")),
        )
}
