...
use crate::models::literate::varieties::_entities::varieties as ReferenceModel;
use crate::models::literate::varieties::data_objects::DataObject as ReferenceDataObject;
use crate::models::literate::varieties::validation::varieties::{ModelValidation, ModelValidator};
...
pub fn routes(ctx: &AppContext) -> Routes {
    Routes::new()
        .prefix("api/literate/varieties")
        .add("/list", get(list))
        .add(
            "/store",
            post(store).layer(AuthorizedLayer::new(
                ctx.clone(),
                "literate.varieties.index",
            )),
        )
        // .add(
        //     "/{id}",
