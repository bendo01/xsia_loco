...
use crate::models::literate::categories::_entities::categories as ReferenceModel;
use crate::models::literate::categories::data_objects::DataObject as ReferenceDataObject;
use crate::models::literate::categories::validation::categories::{
...
pub fn routes(ctx: &AppContext) -> Routes {
    Routes::new()
        .prefix("api/literate/categories")
        .add("/list", get(list))
        .add(
            "/store",
            post(store).layer(AuthorizedLayer::new(
                ctx.clone(),
                "literate.categories.index",
            )),
        )
        // .add(
        //     "/{id}",
