...
use crate::models::auth::permissions::_entities::permissions as ReferenceModel;
use crate::models::auth::permissions::data_objects::DataObject as ReferenceDataObject;
use crate::models::auth::permissions::validation::permissions::{ModelValidation, ModelValidator};
...
pub async fn update(
    ...
    let now = Utc::now().naive_utc();
    item.is_open = Set(payload.is_open);
...
pub fn routes(ctx: &AppContext) -> Routes {
    Routes::new()
        .prefix("api/auth/permissions")
        .add("/list", get(list))
        .add(
            "/store",
            post(store).layer(AuthorizedLayer::new(ctx.clone(), "auth.permissions.index")),
        )
        // .add(
        //     "/{id}",
        //     get(show).layer(AuthorizedLayer::new(ctx.clone(), "auth.permissions.index")),
        // )
        .add(
            "/{id}",
            delete(destroy).layer(AuthorizedLayer::new(ctx.clone(), "auth.permissions.index")),
        )
        .add(
            "/{id}",
            put(update).layer(AuthorizedLayer::new(ctx.clone(), "auth.permissions.index")),
        )
        .add(
            "/",
            post(index).layer(AuthorizedLayer::new(ctx.clone(), "auth.permissions.index")),
        )
}
