...
use crate::models::institution::reference::categories::data_objects::DataObject as ReferenceDataObject;
use crate::models::institution::reference::categories::validation::categories::{
...
pub fn routes(ctx: &AppContext) -> Routes {
    Routes::new()
        .prefix("api/institution/reference/categories")
        .add("/list", get(list))
        .add(
            "/store",
            post(store).layer(AuthorizedLayer::new(
                ctx.clone(),
                "institution.reference.categories.index",
            )),
        )
        .add(
            "/{id}",
