...
impl ModelValidator {
    ...
    pub fn from_model(model: &ReferenceModel::Model) -> Self {
        Self {
            ...
            archive_id: model.archive_id.clone(),
    ...
    pub fn from_active_model(model: &ReferenceModel::ActiveModel) -> Self {
        Self {
            ...
            archive_id: model.archive_id.as_ref().to_owned(),
        ...
        let mut query = ReferenceModel::Entity::find()
            ...
            .filter(
                model::query::condition()
                    .eq(ReferenceModel::Column::EvaluationId, self.evaluation_id)
                    .build(),
            )
            .filter(
                model::query::condition()
                    .eq(ReferenceModel::Column::ArchiveId, self.archive_id)
                    .build(),
            )
            .filter(
                model::query::condition()
