...
// use chrono::{offset::Local, Duration, Utc};
...
impl ActiveModel {
    ...
    pub async fn set_email_verification_sent(
        ...
        let now = Utc::now().naive_utc();
        self.email_verification_sent_at = ActiveValue::set(Some(now));
    ...
    pub async fn verified(mut self, db: &DatabaseConnection) -> ModelResult<Model> {
        ...
        // Utc::now().naive_utc();
        // self.email_verified_at = Utc::now().naive_utc();
        let now = Utc::now().naive_utc();
        self.email_verified_at = Set(Some(now));
        Ok(self.update(db).await?)
    }
    ...
    pub async fn create_magic_link(mut self, db: &DatabaseConnection) -> ModelResult<Model> {
        ...
        let now = Utc::now().naive_utc();
        let expired = now + Duration::minutes(MAGIC_LINK_EXPIRATION_MIN.into());
        self.magic_link_token = ActiveValue::set(Some(random_str));
        // self.magic_link_expiration = ActiveValue::set(Some(expired.into()));
        self.magic_link_expiration = ActiveValue::set(Some(expired));
        Ok(self.update(db).await?)
    }
