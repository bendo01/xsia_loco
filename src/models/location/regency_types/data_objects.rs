...
pub struct LocationRegencyTypeDataObject {
    pub regency_type: LocationRegencyType::Model,
}

impl LocationRegencyTypeDataObject {
    ...
    pub async fn get_by_id(ctx: &AppContext, id: Uuid) -> Result<Option<Self>> {
        let regency_type = LocationRegencyType::Entity::find_by_id(id)
            .filter(LocationRegencyType::Column::DeletedAt.is_null())
            .one(&ctx.db)
            .await?;
        // if let Some(regency_type) = regency_type {
        //     Ok(Some(Self { regency_type }))
        // } else {
        //     Ok(None)
        // }
        regency_type.map_or_else(|| Ok(None), |regency_type| Ok(Some(Self { regency_type })))
    }
}
