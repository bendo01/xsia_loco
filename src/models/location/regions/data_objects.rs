...
pub struct LocationRegionDataObject {
    pub region: LocationRegion::Model,
}

impl LocationRegionDataObject {
    ...
    pub async fn get_by_id(ctx: &AppContext, id: Uuid) -> Result<Option<Self>> {
        let region = LocationRegion::Entity::find_by_id(id)
        ...
        //     Ok(None)
        // }
        region.map_or_else(|| Ok(None), |region| Ok(Some(Self { region })))
    }
}
