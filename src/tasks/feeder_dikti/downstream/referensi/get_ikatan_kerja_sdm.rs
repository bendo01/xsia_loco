...
#[derive(Clone, Debug, Serialize, Deserialize)]
pub struct GetIkatanKerjaSDM;

impl GetIkatanKerjaSDM {
    ...
    pub async fn upsert(ctx: &AppContext, params: Data) -> Result<(), Error> {
        ...
        let data_result = FeederReferensiIkatanKerjaSumberDayaManusia::Entity::find()
            .filter(FeederReferensiIkatanKerjaSumberDayaManusia::Column::DeletedAt.is_null())
            .filter(
                FeederReferensiIkatanKerjaSumberDayaManusia::Column::IdIkatanKerja
                    .eq(params.id_ikatan_kerja.clone()),
            )
            .filter(
                FeederReferensiIkatanKerjaSumberDayaManusia::Column::NamaIkatanKerja.eq(params
                    .nama_ikatan_kerja
                    .clone()
                    .trim()
                    .to_string()),
        ...
        if let Some(existing_reference) = data_opt {
            let mut reference: FeederReferensiIkatanKerjaSumberDayaManusia::ActiveModel =
                existing_reference.into();
            reference.nama_ikatan_kerja = Set(params.nama_ikatan_kerja.clone().trim().to_string());
            ...
            let new_reference = FeederReferensiIkatanKerjaSumberDayaManusia::ActiveModel {
                id: Set(pk_id),
                id_ikatan_kerja: Set(params.id_ikatan_kerja.clone()),
                nama_ikatan_kerja: Set(params.nama_ikatan_kerja.clone().trim().to_string()),
            ...
            match FeederReferensiIkatanKerjaSumberDayaManusia::Entity::insert(new_reference)
                .exec(&ctx.db)
                .await
            {
...
#[async_trait]
impl Task for GetIkatanKerjaSDM {
    fn task(&self) -> TaskInfo {
        TaskInfo {
            name: "GetIkatanKerjaSDM".to_string(),
            detail: "get ikatan kerja sdm data from feeder".to_string(),
        }
    }
