...
const TASK_NAME: &str = "EstimatePerkuliahanMahasiswa";
...
pub struct EstimatePerkuliahanMahasiswa;

impl EstimatePerkuliahanMahasiswa {
    ...
    async fn enqueue_worker(
        ...
        let worker_args = crate::workers::feeder_dikti::downstream::master::upsert::get_list_perkuliahan_mahasiswa::WorkerArgs {
            records,
        };

        match crate::workers::feeder_dikti::downstream::master::upsert::get_list_perkuliahan_mahasiswa::Worker::perform_later(app_context, worker_args).await {
            Ok(_) => {
...
#[async_trait]
impl Task for EstimatePerkuliahanMahasiswa {
    fn task(&self) -> TaskInfo {
        TaskInfo {
            name: TASK_NAME.to_string(),
            detail: "Fetch and process List Perkuliahan Mahasiswa data from Feeder Dikti"
                .to_string(),
        }
    }
