...
const API_ACTION: &str = "GetBiodataDosen";
...
impl EstimateBiodataDosen {
    ...
    async fn enqueue_worker(
        ...
        use crate::models::feeder::master::biodata_dosen::feeder_model::ModelInput as FeederModel;
        ...
        let worker_args = crate::workers::feeder_dikti::downstream::master::upsert::get_biodata_dosen::WorkerArgs {
            records,
        };

        match crate::workers::feeder_dikti::downstream::master::upsert::get_biodata_dosen::Worker::perform_later(app_context, worker_args).await {
            Ok(_) => {
...
impl Task for EstimateBiodataDosen {
    fn task(&self) -> TaskInfo {
        TaskInfo {
            ...
            detail: "Fetch and process Biodata Dosen data from Feeder Dikti".to_string(),
        }
    }
