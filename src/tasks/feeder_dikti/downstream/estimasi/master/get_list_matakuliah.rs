...
const TASK_NAME: &str = "EstimateMatakuliah";
...
impl EstimateListMatakuliah {
    ...
    async fn enqueue_worker(
        ...
        use crate::models::feeder::master::matakuliah::feeder_model::ModelInput as FeederModel;
        ...
        let worker_args = crate::workers::feeder_dikti::downstream::master::upsert::get_list_matakuliah::WorkerArgs {
            records,
        };

        match crate::workers::feeder_dikti::downstream::master::upsert::get_list_matakuliah::Worker::perform_later(app_context, worker_args).await {
            Ok(_) => {
...
impl Task for EstimateListMatakuliah {
    fn task(&self) -> TaskInfo {
        TaskInfo {
            ...
            detail: "Fetch and process List Mata Kuliah data from Feeder Dikti".to_string(),
        }
    }
