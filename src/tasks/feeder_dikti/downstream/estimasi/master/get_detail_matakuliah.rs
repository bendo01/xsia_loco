...
impl EstimateDetailMatakuliah {
    ...
    async fn enqueue_worker(
        ...
        use crate::models::feeder::master::matakuliah::feeder_model::ModelInputDetailMatakuliah as FeederModel;
        ...
        let worker_args = crate::workers::feeder_dikti::downstream::master::upsert::get_detail_matakuliah::WorkerArgs {
            records,
        };

        match crate::workers::feeder_dikti::downstream::master::upsert::get_detail_matakuliah::Worker::perform_later(app_context, worker_args).await {
            Ok(_) => {
...
impl Task for EstimateDetailMatakuliah {
    fn task(&self) -> TaskInfo {
        TaskInfo {
            ...
            detail: "Fetch and process Detail Mata Kuliah data from Feeder Dikti".to_string(),
        }
    }
