use crate::workers::feeder_dikti::downstream::master::download::get_riwayat_nilai_mahasiswa::{
    Worker, WorkerArgs,
};
...
pub struct ExecuteWorkerGetRiwayatNilaiMahasiswa;
#[async_trait]
impl Task for ExecuteWorkerGetRiwayatNilaiMahasiswa {
    fn task(&self) -> TaskInfo {
        TaskInfo {
            name: "ExecuteWorkerGetRiwayatNilaiMahasiswa".to_string(),
    ...
    async fn run(&self, app_context: &AppContext, _vars: &task::Vars) -> Result<()> {
        ...
        let data_result = FeederAkumulasiJumlahData::Entity::find()
            ...
            .filter(FeederAkumulasiJumlahData::Column::Name.eq("FA0006GetCountRiwayatNilaiMahasiswa".to_string()))
            .one(&app_context.db)
            .await;
        ...
        if let Some(existing_reference) = data_opt {
            ...
            for offset in (0..total_feeder).step_by(limit as usize) {
                let worker_args = WorkerArgs {
                    act: "GetRiwayatNilaiMahasiswa".to_string(),
                    filter: None,
                ...
                match Worker::perform_later(app_context, worker_args).await {
                    Ok(_) => {
                        println!("✅ Enqueued worker for GetRiwayatNilaiMahasiswa: limit {} offset: {}", limit, offset);
                    }
                    Err(err) => {
        ...
        println!("Task ExecuteWorkerGetRiwayatNilaiMahasiswa completed - all workers enqueued");
        Ok(())
    }
}
