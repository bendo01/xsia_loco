...
impl Task for ExecuteWorkerGetNilaiTransferPendidikanMahasiswa {
    ...
    async fn run(&self, app_context: &AppContext, _vars: &task::Vars) -> Result<()> {
        ...
        let data_result = FeederAkumulasiJumlahData::Entity::find()
            ...
            .filter(
                FeederAkumulasiJumlahData::Column::Name
                    .eq("FA0005GetCountNilaiTransferPendidikanMahasiswa".to_string()),
            )
            .one(&app_context.db)
            .await;
        ...
        if let Some(existing_reference) = data_opt {
            ...
            for offset in (0..total_feeder).step_by(limit as usize) {
                ...
                match Worker::perform_later(app_context, worker_args).await {
                    Ok(_) => {
                        println!(
                            "✅ Enqueued worker for GetNilaiTransferPendidikanMahasiswa: limit {} offset: {}",
                            limit, offset
                        );
                    }
                    Err(err) => {
        ...
        println!(
            "Task ExecuteWorkerGetNilaiTransferPendidikanMahasiswa completed - all workers enqueued"
        );
        Ok(())
    }
}
