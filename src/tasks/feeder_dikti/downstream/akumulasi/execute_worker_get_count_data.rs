use crate::workers::feeder_dikti::downstream::akumulasi::get_count_data::{
    GetCountDataWorker, GetCountDataWorkerWorkerArgs,
};
use loco_rs::prelude::*;
...
impl Task for ExecuteWorkerGetCountData {
    ...
    async fn run(&self, app_context: &AppContext, _vars: &task::Vars) -> Result<()> {
        ...
        let data_combinations = vec![
            (
                "FA0001GetCountAktivitasMahasiswa",
                "GetCountAktivitasMahasiswa",
            ),
            (
                "FA0002GetCountPrestasiMahasiswa",
                "GetCountPrestasiMahasiswa",
            ),
            ("FA0003GetCountMahasiswa", "GetCountMahasiswa"),
            (
                "FA0004GetCountRiwayatPendidikanMahasiswa",
                "GetCountRiwayatPendidikanMahasiswa",
            ),
            (
                "FA0005GetCountNilaiTransferPendidikanMahasiswa",
                "GetCountNilaiTransferPendidikanMahasiswa",
            ),
            (
                "FA0006GetCountRiwayatNilaiMahasiswa",
                "GetCountRiwayatNilaiMahasiswa",
            ),
            ("FA0007GetCountDosen", "GetCountDosen"),
            (
                "FA0008GetCountPenugasanSemuaDosen",
                "GetCountPenugasanSemuaDosen",
            ),
            (
                "FA0009GetCountAktivitasMengajarDosen",
                "GetCountAktivitasMengajarDosen",
            ),
            ("FA0010GetCountSkalaNilaiProdi", "GetCountSkalaNilaiProdi"),
            (
                "FA0011GetCountPeriodePerkuliahan",
                "GetCountPeriodePerkuliahan",
            ),
            ...
            (
                "FA0017GetCountNilaiPerkuliahanKelas",
                "GetCountNilaiPerkuliahanKelas",
            ),
            ...
            (
                "FA0021GetCountDosenPengajarKelasKuliah",
                "GetCountDosenPengajarKelasKuliah",
            ),
            ("FA0022GetCountMahasiswaLulusDO", "GetCountMahasiswaLulusDO"),
            (
                "FA0023GetCountPesertaKelasKuliah",
                "GetCountPesertaKelasKuliah",
            ),
            (
                "FA0024GetCountPerkuliahanMahasiswa",
                "GetCountPerkuliahanMahasiswa",
            ),
            (
                "FA0025GetCountMahasiswaBimbinganDosen",
                "GetCountMahasiswaBimbinganDosen",
            ),
            (
                "FA0026GetCountKonversiKampusMerdeka",
                "GetCountKonversiKampusMerdeka",
            ),
            (
                "FA0027GetCountRencanaPembelajaran",
                "GetCountRencanaPembelajaran",
            ),
        ...
        println!(
            "Enqueueing {} GetCountDataWorker jobs",
            data_combinations.len()
        );
        ...
        for (table_name, action_name) in data_combinations {
            ...
            match GetCountDataWorker::perform_later(app_context, worker_args).await {
                Ok(_) => {
                    println!(
                        "✅ Enqueued worker for table: {} with action: {}",
                        table_name, action_name
                    );
                }
                Err(err) => {
                    eprintln!(
                        "❌ Failed to enqueue worker for table: {} with action: {}. Error: {}",
