...
#[async_trait]
impl Task for GetCountData {
    fn task(&self) -> TaskInfo {
        TaskInfo {
            name: "GetCountData".to_string(),
            detail: "get periode estimate count data from feeder".to_string(),
        }
    }

    async fn run(&self, ctx: &AppContext, vars: &task::Vars) -> Result<(), Error> {
        let table_column_name = match vars.cli_arg("table_column_name") {
            Ok(name) => name.clone(),
            Err(err) => {
                eprintln!("Error getting table_column_name: {}", err);
                return Err(err);
            }
        };
        let action_name = match vars.cli_arg("action_name") {
            Ok(name) => name.clone(),
            Err(err) => {
                eprintln!("Error getting action_name: {}", err);
                return Err(err);
            }
        };
