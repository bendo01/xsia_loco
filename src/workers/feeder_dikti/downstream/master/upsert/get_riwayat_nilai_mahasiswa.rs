...
use crate::models::feeder::master::riwayat_nilai_mahasiswa::{
    _entities::riwayat_nilai_mahasiswa, feeder_model::ModelInput,
};
...
impl BackgroundWorker<WorkerArgs> for Worker {
    ...
    ///
    /// This function processes a batch of riwayat nilai mahasiswa records:
        ...
        for (index, record) in args.records.iter().enumerate() {
            match Self::upsert_record(&self.ctx, record).await {
                ...
                Err(e) => {
                    error_count += 1;
                    eprintln!(
                        "  ❌ Record {}/{}: Failed - nim={:?}, id_kelas={:?}, id_matkul={:?}, error: {}",
                        index + 1,
                        args.records.len(),
                        record.nim,
                        record.id_kelas,
